...
/**
 * Copies the previously-generated Pagefind search index from `_site/figma/pagefind/`
 * into `public/figma/pagefind/` so `astro dev` can serve search at `/figma/pagefind/`,
...
function copyPagefindIndex() {
  const source = path.join(process.cwd(), '_site', 'figma', 'pagefind')
  if (!fs.existsSync(source)) return
  const destination = path.join(getDocsPublicFsPath(), 'figma', 'pagefind')
